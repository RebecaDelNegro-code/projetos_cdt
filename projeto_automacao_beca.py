...
st.set_page_config(page_title="Sorveteria Glacê", page_icon="🍦")
...
col4.metric("ESTOQUE BAIXO", baixos)
...
aba_vendas, aba_estoque, aba_extrato = st.tabs(["💰 Vendas", "📦 Estoque", "📋 Extrato"])
...
with aba_vendas:
    ...
    with st.form("form_venda"):
        ...
        if btn_vender:
            ...
            if qtd > estoque:
                st.error(f"Estoque insuficiente. Temos apenas {estoque} unidades.")
...
with aba_estoque:
    ...
    with c2:
        qtd_repor = st.number_input("Qtd:", min_value=1, value=10)
    with c3:
        st.write("")
        st.write("")
...
with aba_extrato:
    ...
    dados_exportacao = {
        "sorveteria": "Sorveteria Glacê",
        "caixa": round(st.session_state.caixa, 2),
        "vendas_dia": st.session_state.vendas_dia,
        "itens_vendidos": st.session_state.itens_vendidos,
        "sabores": st.session_state.sabores,
        "historico": st.session_state.historico
    }
    
    st.download_button(
        label="📤 Exportar JSON",
        data=json.dumps(dados_exportacao, ensure_ascii=False, indent=4),
        file_name="sorveteria_glace.json",
